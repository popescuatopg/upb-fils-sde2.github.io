...
## Resources
 ...
 2. [Tour of Rust](https://tourofrust.com/) tutoriel étape par étape
...
| [`alloc`](https://doc.rust-lang.org/alloc/index.html) | Fournit tout, depuis le niveau `core` ainsi que les structures de données allouées *heap* comme `Box` et `Vec`. Le développeur doit fournir une mémoire allouée, comme [embedded_alloc](https://docs.rs/embedded-alloc/latest/embedded_alloc/). | Memory Allocator |
| [`std`](https://doc.rust-lang.org/std/index.html) | Fournit tout, depuis le niveau `alloc`, ainsi que de nombreuses fonctionnalités qui dépendent de la plate-forme, y compris les threads et les E/S. Il s'agit du niveau par défaut pour les applications Windows, Linux, macOS et systèmes d'exploitation similaires. | Operating System |
...
```
:::note 
Pour comprendre mieux, lire le [chapitre 5](https://jimskapt.github.io/rust-book-fr/ch05-00-structs.html) qui se trouve dans la documentation au début du TP!
:::

### Enums
Les énumérations, également appelées `enum`, vous permettent de définir un type en énumérant ses variantes possibles.
Comment définir une énumération :
...
### Exercises
:::note
Avant d'aborder les exercices, jetez un œil et parcourez les chapitres [1](https://tourofrust.com/chapter_1_en.html), [2](https://tourofrust.com/chapter_2_en.html) et [3](https://tourofrust.com/chapter_3_en.html) des tutoriels [Tour of Rust](https://tourofrust.com/).
:::
