...
## Resources
 1. [Recoverable Errors with Result](https://jimskapt.github.io/rust-book-fr/ch09-02-recoverable-errors-with-result.html)
...
2. Écrivez un programme qui reçoit en argument de ligne de commande un fichier et une tâche. Mettre en œuvre les tâches :
    a. **print** - imprime le contenu du fichier et le file descriptor
    b. **size** - imprime la taille du fichier'
    c. **owner** - affiche l'utilisateur (uid) et le groupe (gid) du fichier
    d. **mode_number** - affiche le mode (permissions) en numero du fichier
    e. **type** - le type de fichier: dossier, fichier, lien, périphérique caractère, périphérique bloc, …
    f. **mode_text** - affiche le mode (permissions) en text (rwx…) du fichier
    g. **print_buffer** - imprimez le contenu du fichier à l'aide des fonctions open et read.
